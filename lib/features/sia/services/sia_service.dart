...
class SiaService extends GetxService {
  ...
  Future<void> loadSiaConfiguration() async {
    ...
    if (backupOption == 'SecureSphere') {
      await _loadManagedNodeFromBackend();
  ...
  Future<bool> _checkIfUserHasBackendConfig() async {
    // Path A: check local secure storage instead of backend.
    try {
      final decSvc = Get.find<DecentralizedService>();
      if (decSvc.isDecentralized) {
        final config = await decSvc.getNodeConfig();
        return config != null;
      }
    } catch (_) {}
  ...
  /// Load custom config from backend (or local storage for Path A)
  Future<void> _loadCustomConfigFromBackend() async {
    // Path A: load from flutter_secure_storage, no network call.
    try {
      final decSvc = Get.find<DecentralizedService>();
      if (decSvc.isDecentralized) {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final portStr = nodeData['port']?.toString() ?? '';
  ...
  /// Load SecureSphere-managed node config from backend env vars.
  Future<void> _loadManagedNodeFromBackend() async {
    try {
      final authService = Get.find<AuthService>();
      final headers = await authService.getAuthHeaders();

      final response = await Get.find<AuthenticatedClient>()
          .get(Uri.parse(ApiConfig.managedNodeEndpoint), headers: headers)
          .timeout(const Duration(seconds: 10));
      ...
      if (response.statusCode == 200) {
        ...
        if (data['success'] == true) {
          ...
          final port = data['port'] as String? ?? '';
          ...
          // Store password locally encrypted so it survives cold restarts.
  ...
  /// Save SIA config to backend (or local storage for Path A)
  Future<bool> saveSiaConfigToBackend(String host, String port, String password) async {
    // Path A: save to secure storage only — no backend call.
    try {
      final decSvc = Get.find<DecentralizedService>();
      if (decSvc.isDecentralized) {
        await decSvc.saveNodeConfig(host, port, password);
        _currentConfig = SiaNodeConfig(
          host: host,
          port: port,
          password: password,
  ...
  Future<bool> isPasswordMissing() async {
    // Path A: password lives in node_password via DecentralizedService — never missing if config loaded
    try {
      final decSvc = Get.find<DecentralizedService>();
      if (decSvc.isDecentralized) {
        return _currentConfig == null || _currentConfig!.password.isEmpty;
      }
    } catch (_) {}

    // Path B: managed users — check sia_password key as before
    final prefs = await SharedPreferences.getInstance();
    final backupOption = prefs.getString('backupOption') ?? 'SecureSphere';
    if (backupOption == 'SecureSphere') return false;
  ...
  Future<void> clearAllPasswordData() async {
    // Never wipe password data for decentralized users — their password is in node_password
    try {
      final decSvc = Get.find<DecentralizedService>();
