...
class RenterdUploader {
  ...
  /// Guards against building URLs with an empty host (unconfigured node).
  void _assertHostConfigured(SiaConfig config) {
    if (config.ip.isEmpty || config.port.isEmpty) {
      ...
      if (isDecentralized) {
        ...
        throw RenterdUploadException(
            'No Sia node assigned. Please contact support or add a node in Settings.');
      }
    }
  }
  ...
  /// - Self-hosted: user-specific bucket (user-vault-USERID) - S3 compliant naming
  Future<String> _getBucketName() async {
    ...
    if (userId == null || userId.isEmpty) {
      throw Exception('Cannot determine vault bucket: user ID is not available');
    }
    if (backupOption == 'SecureSphere') {
      ...
      // Self-hosted: use user-specific vault bucket (secure isolation)
      final bucketName = 'user-vault-$userId'; // S3-compliant naming (hyphens, no underscores)
      return bucketName;
    }
  }
  ...
  Future<String> uploadFile(
    ...
    try {
      ...
      _assertHostConfigured(siaConfig);
